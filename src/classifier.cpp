...
GNB::GNB() {
  ...
  // Initalize the means and deviation for each process model
  
  left_means = ArrayXd(4);
  left_means << 0,0,0,0;
  left_std_dev = ArrayXd(4);
  left_std_dev << 0,0,0,0;
  left_prior = 0;
    
  keep_means = ArrayXd(4);
  keep_means << 0,0,0,0;
  keep_std_dev = ArrayXd(4);
  keep_std_dev << 0,0,0,0;
  keep_prior = 0;
  
  right_means = ArrayXd(4);
  right_means << 0,0,0,0;
  right_std_dev = ArrayXd(4);
  right_std_dev << 0,0,0,0;
...
void GNB::train(const vector<vector<double>> &data,
  ...
  for (int i=0; i<labels.size(); ++i) {
    ...
    if (labels[i] == "left"){
      left_std_dev += (data_point - left_means)*(data_point - left_means);
    } else if (labels[i] == "keep") {
      keep_std_dev += (data_point - keep_means)*(data_point - keep_means);
    } else if (labels[i] == "right") {
      right_std_dev += (data_point - right_means)*(data_point - right_means);
    }
  }
  
  // Compute standard deviations
  left_std_dev  = (left_std_dev/left_size).sqrt();
  keep_std_dev  = (keep_std_dev/keep_size).sqrt();
  right_std_dev = (right_std_dev/right_size).sqrt();
...
string GNB::predict(const vector<double> &sample) {
  ...
  for (int i=0; i < sample.size() ; ++i) {
    left_p *= (1.0/sqrt(2.0 * M_PI * pow(left_std_dev[i], 2)))
            * exp(-0.5*pow(sample[i] - left_means[i], 2)/pow(left_std_dev[i], 2));
    keep_p *= (1.0/sqrt(2.0 * M_PI * pow(keep_std_dev[i], 2)))
            * exp(-0.5*pow(sample[i] - keep_means[i], 2)/pow(keep_std_dev[i], 2));
    right_p *= (1.0/sqrt(2.0 * M_PI * pow(right_std_dev[i], 2)))
            * exp(-0.5*pow(sample[i] - right_means[i], 2)/pow(right_std_dev[i], 2));
  }
  ...
  // Find the hightest probability from  the list
